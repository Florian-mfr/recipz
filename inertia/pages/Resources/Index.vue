  ...
  <Layout backUrl="/">
    <div class="bg-gray-100 min-h-screen">
      <div class="container mx-auto px-4 py-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Liste des Ressources</h1>
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="resource in filteredResources" :key="resource.id" class="hover:bg-gray-50">
                <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ resource.name }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-gray-900">{{ resource.price ? `${resource.price} K` : 'N/A' }}</td>
...
const filteredResources = computed(() => {
  return props.resources.filter(resource => 
    resource.name.toLowerCase().includes(filter.value.toLowerCase())
  )
})
